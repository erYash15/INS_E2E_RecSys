...
def compute_and_save_content_embeddings(
    content_df, model, save_path=config.content_emb_path
):
    # Split IDs and features
...
if __name__ == "__main__":
    ...
    test_content = preprocess_content_features(
        test_content, save_dir="artifacts/content_encoders"
    )
    print(test_content)
    content_tower_cols = [
        "hashid",
    ...
    print(
        f"Loaded {len(content_emb_dict)} content embeddings from {config.content_emb_path}"
    )
    ...
    for user_id, user_emb in tqdm(
        user_emb_dict.items(), desc="Users", total=len(user_emb_dict)
    ):
        scores = []
    ...
    print(
        f"Saved top 50 recommendations for each user to {config.topk_recommendations_path}"
    )
